class info_share {
    constructor(url, name, address) {
        this.url = url;        // URL of the image
        this.name = name;      // Name of the tourist destination
        this.address = address;// Address of the tourist destination
    }
}
let arr_pass=[];
...
let result;
let see_All = document.querySelector(".see");
...
let response = async () =>{
    for(let i=0;i<TouristDestinations.length;i++){
        ...
        arr_pass.push(new info_share(url_img ,TouristDestinations[i].name, TouristDestinations[i].address))
    }  
}
